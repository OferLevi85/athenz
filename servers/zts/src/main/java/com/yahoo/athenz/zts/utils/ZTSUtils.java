...
public class ZTSUtils {
    ...
    private static final String ATHENZ_PROP_LOCAL_CERT_PATH_FOR_SSL             = "athenz.ssl_local_cert_path";
    private static final String ATHENZ_PROP_LOCAL_KEY_PATH_FOR_SSL              = "athenz.ssl_local_key_path";
    private static final String ATHENZ_PROP_USE_LOCAL_CERT_AND_KEY_FOR_SSL      = "athenz.ssl_use_local_cert_and_key";
    private static final String ATHENZ_DEFAULT_USE_LOCAL_CERT_AND_KEY_FOR_SSL   = "false";
    ...
    public static SSLContext createServerClientSSLContext(PrivateKeyStore privateKeyStore) {
        if (Boolean.parseBoolean(System.getProperty(ATHENZ_PROP_USE_LOCAL_CERT_AND_KEY_FOR_SSL, ATHENZ_DEFAULT_USE_LOCAL_CERT_AND_KEY_FOR_SSL))) {
            return getSslContextAthenzCert(privateKeyStore);
        } else {
            return getSslContextExternalCert(privateKeyStore);
        }
    }

    private static SSLContext getSslContextAthenzCert(PrivateKeyStore privateKeyStore) {
        ...
        final String certPath = System.getProperty(ATHENZ_PROP_LOCAL_CERT_PATH_FOR_SSL);
        if (certPath == null) {
            LOGGER.error("Unable to create client ssl context: no local ssl cert path specified");
            return null;
        }
        if (!new File(certPath).exists()) {
            LOGGER.error("Unable to create client ssl context: local ssl cert not found in {}", certPath);
            return null;
        }
        final String keyPath = System.getProperty(ATHENZ_PROP_LOCAL_KEY_PATH_FOR_SSL);
        if (keyPath == null) {
            LOGGER.error("Unable to create client ssl context: no local ssl key path specified");
            return null;
        }
        if (!new File(keyPath).exists()) {
            LOGGER.error("Unable to create client ssl context: local ssl key not found in {}", keyPath);
            return null;
        }
    ...
    private static SSLContext getSslContextExternalCert(PrivateKeyStore privateKeyStore) {
