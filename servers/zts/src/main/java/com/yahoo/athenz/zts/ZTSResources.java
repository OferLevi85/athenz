...
public class ZTSResources {
    ...
    @Path("/rolecert")
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(description = "Fetch all roles that are tagged as requiring role certificates for service")
    public RoleList getRolesRequireRoleCert(
        @Parameter(description = "", required = true) @QueryParam("service") String service) {
        int code = ResourceException.OK;
        ResourceContext context = null;
        try {
            context = this.delegate.newResourceContext(this.request, this.response, "getRolesRequireRoleCert");
            context.authenticate();
            return this.delegate.getRolesRequireRoleCert(context, service);
        } catch (ResourceException e) {
            code = e.getCode();
            switch (code) {
            case ResourceException.BAD_REQUEST:
